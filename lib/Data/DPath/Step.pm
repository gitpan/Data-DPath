...
# @ISA = qw(DynaLoader);
# 
# bootstrap Data::DPath::Step $VERSION;
...
sub test_nt_comment () { 6; } # text()
sub test_nt_data () { 7; } # text()
...
sub new {
    ...
    $axis_method =~ tr/-/_/;
    my $self = {
        pp => $pp, # the Data::DPath::Parser class
        axis => $axis,
...
sub evaluate {
    ...
    #warn "Eval Step: ", $self -> as_string, "\n";
    #warn "self: ", Data::Dumper -> Dump([$self]);
    
    $self->{pp}->set_context_set($from);
    
    my $initial_nodeset = Data::DPath::NodeSet->new();
    ...
    #warn "Step::evaluate returning nodeset size: ", $initial_nodeset->size, "\n";
        
    #warn "Evaluate Step returning: ", Data::Dumper -> Dump([$initial_nodeset]);
...
sub axis_attribute {
    ...
    # need to handle @isa, @can, @version, @defined, etc.
    #foreach my $attrib (@{$context->getAttributes}) {
    #    if ($self->test_attribute($attrib)) {
    #        $results->push($attrib);
    #    }
    #}
    if($self -> test_attribute($context)) {
        $results -> push($context);
    }
}
...
sub axis_child {
    ...
    #warn "Method axis_child: ", Data::Dumper -> Dump([$context]);
    #warn "Context ", Data::Dumper -> Dump([$context]);
    
    foreach my $node (@{$context->getChildNodes}) {
        #warn "Testing ", Data::Dumper -> Dump([$node]);
...
sub axis_method {
    ...
    # if $context -> can($method), then push that
    # otherwise, if $context -> {$method} is CODE, then push that
    my $method;

    my $code = UNIVERSAL::can($context -> data, $method)
            || UNIVERSAL::isa($context -> data, 'HASH') 
               && exists($context -> data -> {$method})
               && UNIVERSAL::isa($context -> data -> {$method}, 'CODE')
               && $context -> data -> {$method};

    $results -> push(Data::DPath::Node($context -> path . '/method::' . $method, 
                                       $code) 
                    ) if defined $code;
}
...
sub node_test {
    ...
    if ($test == test_ncwild) {
        ...
        #warn "test is `test_qname'\n";
        return unless $node->isElementNode;
        #warn "isElementNode is true\n";
        if ($self->{literal} =~ /:/) {
...
#                warn "match: '$self->{literal}' match NS: '$match_ns' got NS: '", $node_nsnode->getValue, "'\n";
            ...
            #warn "  against: ", $self -> {literal}, "\n";
            return 1 if $node->getName eq $self->{literal};
        }
    }
    elsif ($test == test_nt_data) {
        #warn "test is `test_nt_data'\n";
        return 1 if $node->isTextNode;
    }
    elsif ($test == test_nt_comment) {
        #warn "test is `test_nt_comment'\n";
...
#    warn "Filter by predicate: $predicate\n";
    ...
    for(my $i = 1; $i <= $nodeset->size; $i++) {
        ...
        if ($result->isa('Data::DPath::Boolean')) {
            if ($result->value) {
                $newset->push($nodeset->get_node($i));
            }
        }
        elsif ($result->isa('Data::DPath::Number')) {
            if ($result->value == $i) {
                $newset->push($nodeset->get_node($i));
            }
        }
