...
use Data::DPath::Node::Namespace;
use Data::DPath::Node::Text;
use Data::DPath::Node::PI;
use Data::DPath::Node::Comment;
...
sub start_document {
    ...
    my $document = Data::DPath::Node::Element->new();
    my $newns = Data::DPath::Node::Namespace->new('xml', $xml_ns);
...
sub characters {
    ...
    my $node = Data::DPath::Node::Text->new($text);
...
sub start_element {
    ...
    my $node = Data::DPath::Node::Element->new($tag, $prefix);
...
#            warn "Creating NS node: $prefix = $value\n";
            my $newns = Data::DPath::Node::Namespace->new($prefix, $value);
	    ...
	    my ($prefix, $namespace) = $self->_namespace($name);
            ...
            my $newattr = Data::DPath::Node::Attribute->new($name, $value, $prefix);
...
sub processing_instruction {
    my $self = shift;
    my $pi = shift;
    my $node = Data::DPath::Node::PI->new($pi->{Target}, $pi->{Data});
    $self->{current}->appendChild($node, 1);
}
...
sub comment {
    my $self = shift;
    my $comment = shift;
    my $node = Data::DPath::Node::Comment->new($comment->{Data});
    $self->{current}->appendChild($node, 1);
}
...
C<Data::DPath::Builder> is a SAX handler for building an Data::DPath
tree.

C<Data::DPath::Builder> is used by creating a new instance of
C<Data::DPath::Builder> and providing it as the Handler for a SAX
