package Data::DPath::Function;
use Data::DPath::Number;
use Data::DPath::Literal;
use Data::DPath::Boolean;
use Data::DPath::NodeSet;
use Data::DPath::Node::Attribute;
use strict;
...
sub evaluate {
    ...
    #warn "Eval Function: ", $self -> as_string, "\n";

    if ($node->isa('Data::DPath::NodeSet')) {
    ...
    my $results = $self->_execute($self->{name}, $node, @params);
    #warn "Evaluate Function returning: ", Data::Dumper -> Dump([$results]);
}
...
sub last {
    ...
    return Data::DPath::Number->new($self->{pp}->get_context_size);
}
...
sub position {
    ...
    return Data::DPath::Number->new($self->{pp}->get_context_pos);
}

sub count {
    my $self = shift;
    my ($node, @params) = @_;
    die "count: Parameter must be a NodeSet\n" unless $params[0]->isa('Data::DPath::NodeSet');
    return Data::DPath::Number->new($params[0]->size);
}
...
sub id {
    ...
    my $results = Data::DPath::NodeSet->new();
    if ($params[0]->isa('Data::DPath::NodeSet')) {
        ...
        foreach my $node ($params[0]->get_nodelist) {
            ...
            $results->append($self->id($node, Data::DPath::Literal->new($string)));
        }
    }
...
sub local_name {
    ...
    return Data::DPath::Literal->new($node->getLocalName);
}
...
sub name {
    ...
    return Data::DPath::Literal->new($node->getName);
}
...
sub string {
    ...
    if (@params) {
        return Data::DPath::Literal->new($params[0]->string_value);
    }
    
    # TODO - this MUST be wrong! - not sure now. -matt
    return Data::DPath::Literal->new($node->string_value);
...
sub concat {
    ...
    return Data::DPath::Literal->new($string);
}
...
sub starts_with {
    ...
    if (substr($string1, 0, length($string2)) eq $string2) {
        return Data::DPath::Boolean->True;
    }
    return Data::DPath::Boolean->False;
}
...
sub substring_before {
    ...
    if ($self->contains($node, @params)->value) {
        return Data::DPath::Literal->new($1); # hope that works!
    }
    else {
        return Data::DPath::Literal->new('');
    }
}
...
sub substring_after {
    ...
    die "starts-with: incorrect number of params\n" unless @params == 2;
    if ($self->contains($node, @params)->value) {
        return Data::DPath::Literal->new($2);
    }
    else {
        return Data::DPath::Literal->new('');
    }
...
sub substring {
    ...
    return Data::DPath::Literal->new(substr($str, $offset, $len));
}
...
sub string_length {
    ...
    if (@params) {
        return Data::DPath::Number->new(length($params[0]->string_value));
    }
    else {
        return Data::DPath::Number->new(
...
sub normalize_space {
    ...
    $str =~ s/\s*$//;
    $str =~ s/\s+/ /g;
    return Data::DPath::Literal->new($str);
}
...
sub translate {
    ...
    return Data::DPath::Literal->new($_);
}
...
sub not {
    my $self = shift;
    my ($node, @params) = @_;
    $params[0] = $params[0]->to_boolean unless $params[0]->isa('Data::DPath::Boolean');
    $params[0]->value ? Data::DPath::Boolean->False : Data::DPath::Boolean->True;
}
...
sub number {
    ...
    if (@params) {
        if ($params[0]->isa('Data::DPath::Node')) {
            return Data::DPath::Number->new(
    ...
    return Data::DPath::Number->new( $node->string_value );
}
...
sub sum {
    ...
    return Data::DPath::Number->new($sum);
}
