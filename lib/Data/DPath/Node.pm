...
sub XMLescape {
    ...
    if ($Data::DPath::EncodeUtf8AsEntity) {
...
sub new {
    ...
    my $impl = $class . "Impl";
    my $this = $impl->new(@_);
    #warn "new $class: ", Data::Dumper -> Dump([$this]), "\n";
    if ($Data::DPath::SafeMode) {
        return $this;
    }
    my $self = \$this;
...
sub getNodeType {
    my $self = shift;
    return Data::DPath::Node::UNKNOWN_NODE;
}
...
sub getParentNode {
    my $self = shift;
    return $self->[Data::DPath::Node::node_parent];
}
...
#    warn "getElementById: $id\n";
    ...
    my $node = $root->[Data::DPath::Node::node_ids]{$id};
...
sub get_pos {
    my $self = shift;
    return $self->[Data::DPath::Node::node_pos];
}

sub set_pos {
    my $self = shift;
    $self->[Data::DPath::Node::node_pos] = shift;
}

sub get_global_pos {
    my $self = shift;
    return $self->[Data::DPath::Node::node_global_pos];
}

sub set_global_pos {
    my $self = shift;
    $self->[Data::DPath::Node::node_global_pos] = shift;
}
...
sub insertAfter {
    ...
    my $pos_number = eval { $posnode->[Data::DPath::Node::node_children][-1]->get_global_pos() + 1; };
    ...
    splice @{$self->[Data::DPath::Node::node_children]}, $pos + 1, 0, $newnode;
    
    for (my $i = $pos + 1; $i < @{$self->[Data::DPath::Node::node_children]}; $i++) {
        $self->[Data::DPath::Node::node_children][$i]->set_pos($i);
    }
...
sub insertBefore {
    ...
    $newnode->setParentNode($self);
    splice @{$self->[Data::DPath::Node::node_children]}, $pos, 0, $newnode;
    
    for (my $i = $pos; $i < @{$self->[Data::DPath::Node::node_children]}; $i++) {
        $self->[Data::DPath::Node::node_children][$i]->set_pos($i);
    }
...
sub getPreviousSibling {
    my $self = shift;
    my $pos = $self->[Data::DPath::Node::node_pos];
    return unless $self->[Data::DPath::Node::node_parent];
    return $self->[Data::DPath::Node::node_parent]->getChildNode($pos);
}
...
sub getNextSibling {
    ...
    my $pos = $self->[Data::DPath::Node::node_pos];
    return unless $self->[Data::DPath::Node::node_parent];
    return $self->[Data::DPath::Node::node_parent]->getChildNode($pos + 2);
}

sub setParentNode {
    my $self = shift;
    my $parent = shift;
#    warn "SetParent of ", ref($self), " to ", $parent->[Data::DPath::Node::node_name], "\n";
    $self->[Data::DPath::Node::node_parent] = $parent;
}

sub del_parent_link {
    my $self = shift;
    $self->[Data::DPath::Node::node_parent] = undef;
}
...
sub to_number {
    ...
    return Data::DPath::Number->new($num);
}

sub find {
    my $node = shift;
    my ($path) = @_;
    my $xp = Data::DPath->new(); # new is v. lightweight
...
Data::DPath::Node - internal representation of a node
...
XML::DOM programming to Data::DPath. Compatibility with DOM may
...
Data::DPath::Node. The constants have the same numeric value as the
XML::DOM versions.
...
L<Data::DPath::Node::Element>

=item *

L<Data::DPath::Node::Attribute>

=item *

L<Data::DPath::Node::Namespace>

=item *

L<Data::DPath::Node::Text>

=item *

L<Data::DPath::Node::Comment>

=item *

L<Data::DPath::Node::PI>
