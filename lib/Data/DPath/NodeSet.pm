...
sub push {
	my $self = CORE::shift;
	my (@nodes) = @_;
        #warn "pushing ", scalar(@nodes), " nodes on to ", Data::Dumper -> Dump([$self]);
        #warn "self is of type ", ref($self), "\n";
        #warn "pushing: ", Data::Dumper -> Dump(\@nodes), "\n";
	CORE::push @$self, @nodes;
#        Carp::carp "self now has ", scalar(@$self), " elements";
        #warn "self is now ", Data::Dumper -> Dump([$self]);
}
...
sub to_boolean {
	my $self = CORE::shift;
	return (@$self > 0) ? Data::DPath::Boolean->True : Data::DPath::Boolean->False;
}
...
Data::DPath::NodeSet - a list of XML document nodes

=head1 DESCRIPTION

An Data::DPath::NodeSet object contains an ordered list of nodes. The nodes
each take the same format as described in L<Data::DPath::XMLParser>.
...
Returns a list of nodes. See L<Data::DPath::XMLParser> for the format of
the nodes.
