...
sub evaluate {
    ...
    if (my @predicates = @{$self->{predicates}}) {
        if (!$results->isa('Data::DPath::NodeSet')) {
    ...
    #warn "Evaluate Expr returning: ", Data::Dumper -> Dump([$results]);
...
sub op_equals {
    ...
    if ($lh_results->isa('Data::DPath::NodeSet') &&
            $rh_results->isa('Data::DPath::NodeSet')) {
    ...
    elsif (($lh_results->isa('Data::DPath::NodeSet') ||
            $rh_results->isa('Data::DPath::NodeSet')) &&
            (!$lh_results->isa('Data::DPath::NodeSet') ||
             !$rh_results->isa('Data::DPath::NodeSet'))) {
        ...
        # is true.
        if ($other->isa('Data::DPath::Number')) {
        ...
        elsif ($other->isa('Data::DPath::Literal')) {
            foreach my $node ($nodeset->get_nodelist) {
        ...
        elsif ($other->isa('Data::DPath::Boolean')) {
        ...
        if ($lh_results->isa('Data::DPath::Boolean') ||
            $rh_results->isa('Data::DPath::Boolean')) {
        ...
        elsif ($lh_results->isa('Data::DPath::Number') ||
                $rh_results->isa('Data::DPath::Number')) {
...
sub op_ge {
    ...
    if ($lh_results->isa('Data::DPath::NodeSet') &&
        ...
        foreach my $lhnode ($lh_results->get_nodelist) {
            foreach my $rhnode ($rh_results->get_nodelist) {
                my $lhNum = Data::DPath::Number->new($lhnode->string_value);
                my $rhNum = Data::DPath::Number->new($rhnode->string_value);
        ...
        if ($lh_results->isa('Data::DPath::NodeSet')) {
            ...
            ($false, $true) = (Data::DPath::Boolean->False, Data::DPath::Boolean->True);
        }
        else {
            $nodeset = $rh_results;
            $other = $lh_results;
            # ditto above comment
            ($true, $false) = (Data::DPath::Boolean->False, Data::DPath::Boolean->True);
        }
...
sub op_plus {
    ...
    my $result =
        ...
        $rh_results->to_number->value
            ;
    return Data::DPath::Number->new($result);
}
...
sub op_div {
    ...
    if ($@) {
        ...
        return Data::DPath::Literal->new('Infinity');
    }
    return Data::DPath::Number->new($result);
}
...
sub op_union {
    ...
    if ($lh_result->isa('Data::DPath::NodeSet') &&
            $rh_result->isa('Data::DPath::NodeSet')) {
        my %found;
